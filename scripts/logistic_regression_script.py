...
for degree in xrange(0,maximum_degree):
    ...
    gamma_star, gamma_s, f_s, eta_s = my_driver.learn_map(batch_size=100,
            eta_0=0.1,
            T=5000, alpha=0.01,
            stepsize_update="ada")

    trial_name = 'order_'+str(degree+1)
    sample_results[trial_name] = my_driver.sample(number_samples)
    gamma_results[trial_name] = gamma_s
    f_results[trial_name] = f_s
    eta_results[trial_name] = eta_s
